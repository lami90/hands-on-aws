...
export class Ec2BastionStack extends cdk.Stack {
    constructor(vpcStack: VpcStack, scope: Construct, id: string, props?: cdk.StackProps) {
        ...
        const cfnKeyPair = new ec2.CfnKeyPair(this, 'CfnKeyPair', {
            keyName: 'public-msk-ec2-key-pair',
        })
        ...
        ec2Instance.connections.allowFromAnyIpv4(ec2.Port.tcp(8081));
        ...
        new cdk.CfnOutput(this, 'GetSSHKeyCommand', { value: `touch cdk-key.pem && chmod 777 cdk-key.pem && aws ssm get-parameter --name /ec2/keypair/${cfnKeyPair.getAtt('KeyPairId')} --region ${this.region} --with-decryption --query Parameter.Value --output text > cdk-key.pem && chmod 400 cdk-key.pem` });
